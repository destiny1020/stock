...
df_latest = ts.get_hist_data(symbol, start=start, end=end, ktype='30')
...
try:
    df_previous = pd.read_sql_query('select * from data_30min where code = ' + symbol + ' order by sequence', engine, index_col='date')
...
df.to_sql('data_30min', engine, if_exists='append', index='date')