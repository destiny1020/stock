...
df_latest = ts.get_hist_data(symbol, start=start, end=end, ktype='15')
...
try:
    df_previous = pd.read_sql_query('select * from data_15min where code = ' + symbol + ' order by sequence', engine, index_col='date')
...
df.to_sql('data_15min', engine, if_exists='append', index='date')