...
df_latest = ts.get_hist_data(symbol, start=start, end=end, ktype='W')
...
try:
    df_previous = pd.read_sql_query('select * from data_week where code = ' + symbol + ' order by sequence', engine, index_col='date')
...
df.to_sql('data_week', engine, if_exists='append', index='date')